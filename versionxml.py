...
def _show_python_version(to_file):
    ...
    if not hasattr(sys, 'frozen'):  # check for bzr.exe
        # python executable
        print >>to_file, '<executable>%s</executable>' % sys.executable
    else:
        # pythonXY.dll
        basedir = os.path.dirname(sys.executable)
        python_dll = "python%d%d.dll" % sys.version_info[:2]
        print >>to_file, '<dll>%s</dll>' % os.path.join(basedir, python_dll)
    # and now version of python interpreter
    print >>to_file, '<version>%s</version>' % '.'.join(map(str, sys.version_info))
    print >>to_file, "<standard_library>%s</standard_library>" % os.path.dirname(os.__file__)
    print >>to_file, "</python>"
...
def _show_source_tree(to_file):
    ...
    if src_tree:
        ...
        print >>to_file, "<source_tree>"
        print >>to_file, "<checkout>%s</checkout>" % src_tree.basedir
        print >>to_file, "<revision>%s</revision>" % revno
        print >>to_file, "<revid>%s</revid>" % src_revision_id
        print >>to_file, "<branch_nick>%s</branch_nick>" % src_tree.branch.nick
        print >>to_file, "</source_tree>"
...
def _show_bzr_config(to_file):
    ...
    print >>to_file, "<log_file>%s</log_file>" % trace._bzr_log_filename
